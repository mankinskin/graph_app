...
use super::common::{print_file_location, print_file_location_with_info, format_relative_path};
...
impl CodebaseDuplicationAnalyzer {
    ...
    pub fn with_config(workspace_root: &Path, config: AnalysisConfig) -> Self {
        Self {
    ...
    pub fn analyze_codebase(&mut self) -> Result<DuplicationAnalysis, Box<dyn std::error::Error>> {
        ...
        println!("✅ Analysis completed! Scanned {} functions across {} files.", 
            self.patterns.len(), 
    ...
    fn scan_rust_files(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for entry in std::fs::read_dir(&self.workspace_root)? {
            ...
            if self.should_scan_directory(&path) {
                ...
                if crate_src.exists() {
                    self.scan_directory_recursive(&crate_src, &mut files_scanned)?;
                }
            }
    ...
    fn scan_directory_recursive(&mut self, dir: &Path, files_scanned: &mut usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn analyze_rust_file(&mut self, file_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(parsed_file) = syn::parse_file(&content) {
            for (i, item) in parsed_file.items.iter().enumerate() {
                if let syn::Item::Fn(func) = item {
                    let pattern = self.extract_method_pattern(func, file_path, i + 1);
                ...
                if let syn::Item::Impl(impl_block) = item {
                    for (j, impl_item) in impl_block.items.iter().enumerate() {
                        if let syn::ImplItem::Fn(method) = impl_item {
                            let pattern = self.extract_impl_method_pattern(method, file_path, i + j + 1);
                            self.patterns.push(pattern);
                        }
                    }
                }
    ...
    fn extract_method_pattern(&self, func: &syn::ItemFn, file_path: &Path, line_number: usize) -> MethodPattern {
        let signature = FunctionSignature {
            name: func.sig.ident.to_string(),
            parameter_count: func.sig.inputs.len(),
            parameter_types: func.sig.inputs.iter().map(|arg| {
                match arg {
    ...
    fn extract_impl_method_pattern(&self, method: &syn::ImplItemFn, file_path: &Path, line_number: usize) -> MethodPattern {
        let signature = FunctionSignature {
            name: method.sig.ident.to_string(),
            parameter_count: method.sig.inputs.len(),
            parameter_types: method.sig.inputs.iter().map(|arg| {
                match arg {
                    syn::FnArg::Typed(pat_type) => quote::quote!(#pat_type).to_string(),
                    syn::FnArg::Receiver(_) => "self".to_string(),
                }
    ...
    fn detect_duplications(&self) -> DuplicationAnalysis {
        ...
        for pattern in &self.patterns {
            hash_groups.entry(pattern.code_hash).or_insert_with(Vec::new).push(pattern);
        }
        ...
        let mut signature_groups: HashMap<String, Vec<&MethodPattern>> = HashMap::new();
        for pattern in &self.patterns {
            let key = format!("{}_{}_{}",
            ...
            signature_groups.entry(key).or_insert_with(Vec::new).push(pattern);
        }
        ...
        for (_, group) in signature_groups {
            if group.len() > 1 {
                ...
                let group_hashes: HashSet<u64> = group.iter().map(|p| p.code_hash).collect();
                if group_hashes.len() > 1 {
                    similar_functions.push(group.into_iter().cloned().collect());
                }
            }
        }

        // Detect repeated patterns (same function name in different files)
        let mut name_groups: HashMap<String, Vec<&MethodPattern>> = HashMap::new();
        for pattern in &self.patterns {
            name_groups.entry(pattern.signature.name.clone()).or_insert_with(Vec::new).push(pattern);
        }

        for (name, group) in name_groups {
            if group.len() > 1 {
                repeated_patterns.insert(name, group.into_iter().cloned().collect());
            }
        }

        // Generate refactoring opportunities
        potential_utilities.extend(self.generate_utility_opportunities(&identical_functions));
        potential_utilities.extend(self.generate_parameterization_opportunities(&similar_functions));
    ...
    fn generate_utility_opportunities(&self, identical_groups: &[Vec<MethodPattern>]) -> Vec<RefactoringOpportunity> {
        let mut opportunities = Vec::new();

        for group in identical_groups {
            if group.len() >= 2 && group[0].complexity_score >= self.config.min_complexity_threshold {
                let estimated_lines_saved = group.len().saturating_sub(1) * (group[0].complexity_score as usize);
    ...
    fn generate_parameterization_opportunities(&self, similar_groups: &[Vec<MethodPattern>]) -> Vec<RefactoringOpportunity> {
        let mut opportunities = Vec::new();

        for group in similar_groups {
            if group.len() >= 2 {
                let avg_complexity = group.iter().map(|p| p.complexity_score).sum::<u32>() / group.len() as u32;
                
                // Only suggest parameterization if complexity is worth it
                if avg_complexity >= self.config.min_complexity_threshold {
                    let estimated_lines_saved = (group.len().saturating_sub(1) * avg_complexity as usize) / 2;
    ...
    pub fn print_analysis_results(&self, analysis: &DuplicationAnalysis) {
        println!("\n🎯 COMPREHENSIVE DUPLICATION ANALYSIS RESULTS");
        println!("=====================================");

        // Print identical functions
        if !analysis.identical_functions.is_empty() {
            println!("\n🔄 IDENTICAL FUNCTIONS ({})", analysis.identical_functions.len());
            println!("-------------------------------");
            
            for (i, group) in analysis.identical_functions.iter().enumerate() {
                println!("{}. Function '{}' - {} identical copies:", i + 1, group[0].signature.name, group.len());
                for pattern in group {
                    print_file_location(&pattern.file_path, &self.workspace_root, pattern.line_number);
                }
                println!();
            }
        }
        ...
        if !analysis.similar_functions.is_empty() {
            println!("\n🔀 SIMILAR FUNCTIONS ({})", analysis.similar_functions.len());
            println!("----------------------------");
            
            for (i, group) in analysis.similar_functions.iter().enumerate() {
                println!("{}. Function '{}' - {} similar variants:", i + 1, group[0].signature.name, group.len());
                for pattern in group {
                    print_file_location_with_info(
                        &pattern.file_path, 
                        &self.workspace_root, 
                        pattern.line_number,
                        format!("params: {}", pattern.signature.parameter_count)
                    );
                }
                println!();
            }
        ...
        if !analysis.potential_utilities.is_empty() {
            println!("\n💡 REFACTORING OPPORTUNITIES ({})", analysis.potential_utilities.len());
            println!("----------------------------------");
            
            for (i, opportunity) in analysis.potential_utilities.iter().enumerate() {
                println!("{}. {} (Confidence: {:.0}%)", 
                    i + 1, 
                    opportunity.description,
                    opportunity.confidence * 100.0
                );
                println!("   💾 Estimated lines saved: {}", opportunity.estimated_lines_saved);
                println!("   📍 Suggested location: {}", 
                    format_relative_path(&opportunity.suggested_location, &self.workspace_root)
                );
                println!();
            }
        }
        ...
        // Print summary
        let total_duplicates = analysis.identical_functions.iter().map(|g| g.len().saturating_sub(1)).sum::<usize>();
        let total_estimated_savings = analysis.potential_utilities.iter().map(|o| o.estimated_lines_saved).sum::<usize>();
        
        println!("\n📊 SUMMARY");
        println!("----------");
        println!("• Total duplicate function instances: {}", total_duplicates);
        println!("• Total refactoring opportunities: {}", analysis.potential_utilities.len());
        println!("• Estimated lines that could be saved: {}", total_estimated_savings);
    ...
    fn count_unique_files(&self) -> usize {
        let unique_files: HashSet<&PathBuf> = self.patterns.iter().map(|p| &p.file_path).collect();
        unique_files.len()
    }
}