use crate::utils::duplication_analyzer::{CodebaseDuplicationAnalyzer, DuplicationAnalysis};
use crate::utils::common::{print_file_location_with_info, format_relative_path};
use std::path::Path;
...
impl RefactoringAnalyzer {
    ...
    pub fn analyze_and_recommend(&self, workspace_root: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn get_workspace_name(&self, workspace_root: &Path) -> Option<String> {
    ...
    fn print_specific_recommendations(&self, analysis: &DuplicationAnalysis, workspace_root: &Path) {
        ...
        if !analysis.identical_functions.is_empty() {
            ...
            for (i, group) in analysis.identical_functions.iter().enumerate() {
                if group.len() >= self.config.min_duplicate_threshold {
                    ...
                    let total_lines_saved = group.len().saturating_sub(1) * lines_per_function;
                    
                    println!("{}. Extract '{}' function:", i + 1, function_name);
                    println!("   � {} identical copies found", group.len());
                    println!("   � Estimated lines saved: ~{}", total_lines_saved);
                    println!("   🏗️  Suggested: Create shared utility function");
                    
                    if self.config.verbose {
                        println!("   � Locations:");
                        for pattern in group.iter().take(3) {
                            print_file_location_with_info(&pattern.file_path, workspace_root, pattern.line_number, "");
                        }
                        if group.len() > 3 {
        ...
        // Patterns analysis
        self.print_pattern_analysis(&analysis.repeated_patterns, workspace_root);
    ...
    fn print_pattern_analysis(&self, repeated_patterns: &std::collections::HashMap<String, Vec<crate::utils::duplication_analyzer::MethodPattern>>, workspace_root: &Path) {
        ...
        for (name, patterns) in repeated_patterns {
            if patterns.len() >= self.config.min_duplicate_threshold {
                pattern_count += 1;
                if pattern_count <= 5 {  // Show top 5 patterns
                    println!("• Function name '{}' appears in {} files", name, patterns.len());
                    
                    if self.config.verbose {
                        for pattern in patterns.iter().take(3) {
                            println!("  └─ {}", format_relative_path(&pattern.file_path, workspace_root));
                        }
                        if patterns.len() > 3 {
    ...
    fn print_implementation_suggestions(&self, analysis: &DuplicationAnalysis, workspace_root: &Path) {
        ...
        let estimated_reduction = analysis.potential_utilities.iter()
            .map(|o| o.estimated_lines_saved)
            .sum::<usize>();

        println!("📊 Impact Analysis:");
        println!("   • Exact duplicates: {} function groups", analysis.identical_functions.len());
        println!("   • Similar functions: {} function groups", analysis.similar_functions.len());
        ...
        if estimated_reduction > 0 {
            ...
            println!("\n🎯 Next Steps:");
            println!("   1. Start with exact duplicates (highest impact)");
            println!("   2. Create utility modules for common functions");
            println!("   3. Parameterize similar functions where beneficial");
            println!("   4. Consider creating traits for common behavior patterns");
            
            println!("\n� Suggested Project Structure:");
            println!("   src/");
            println!("   ├── utils/");
            println!("   │   ├── common.rs       (shared utilities)");
            println!("   │   └── extracted/      (extracted functions)");
    ...
    pub fn print_quick_summary(&self, analysis: &DuplicationAnalysis, workspace_root: &Path) {
        let total_duplicates = analysis.identical_functions.iter()
            .map(|g| g.len().saturating_sub(1))
            .sum::<usize>();
        ...
        println!("📉 Total duplicate instances: {}", total_duplicates + total_similar);
        println!();
        
        if total_opportunities > 0 {
            if !analysis.identical_functions.is_empty() {
                println!("🎯 PRIORITY: Extract {} exact duplicate function groups", analysis.identical_functions.len());
            }
            if !analysis.similar_functions.is_empty() {
                println!("⚡ NEXT: Unify {} similar function groups", analysis.similar_functions.len());
            }
            
            let estimated_savings = analysis.potential_utilities.iter()
                .map(|o| o.estimated_lines_saved)
                .sum::<usize>();
            if estimated_savings > 0 {
                println!("💾 IMPACT: ~{} lines could be eliminated", estimated_savings);
            }
        } else {
