...
impl<Side: IndexSide> Pather<Side> {
    ...
    pub fn path_entry_components<S: RelativeSide<Side>>(
        ...
        IndexPathComponents {
            prev_primary,
            primary_exclusive: self
                .index_primary_exclusive::<S, _>(location)
    ...
    pub fn index_components<S: RelativeSide<Side>>(
        &mut self,
        ...
        let primary = primary_exclusive
            .map(|primary_exclusive| {
        ...
        match (
            S::has_secondary_exclusive(&pattern, location.sub_index),
            has_primary_exclusive,
        ) {
            (true, true) => {
                // with secondary exclusive
                // with primary exclusive
                let range = S::primary_range(location.sub_index);
                let (wrapper, pids) = self
                    .graph_mut()
                    .insert_patterns_with_ids([&pattern[range.clone()], &[back, front][..]]);
                self.graph_mut()
                    .replace_in_pattern(location, range, [wrapper]);
                IndexSplitResult {
                    inner: primary,
                    location: location
                        .to_child_location(S::primary_indexed_pos(location.sub_index)),
                    path: vec![ChildLocation {
                        parent: wrapper,
                        pattern_id: pids[1],
                        sub_index: 1,
            ...
            (false, true) => {
                ...
                let pid = self
                    .graph_mut()
                    .add_pattern_with_update(location, [back, front]);
                IndexSplitResult {
                    inner: primary,
                    location: location.to_pattern_location(pid).to_child_location(1),
                    path: vec![],
                }
            }
            (_, false) => {
    ...
    fn child_pattern_offset_splits<S: RelativeSide<Side>>(
        ...
        match child_patterns.try_fold(Vec::with_capacity(len), |mut acc, (pid, pattern)| {
            let (index, inner_offset) =
                Side::token_offset_split(pattern.borrow() as &[Child], offset).unwrap();
            ...
            ControlFlow::Break((pattern, pid, pos)) => Ok(self
                .pattern_perfect_split::<S, _, _>(pattern, ChildLocation::new(parent, pid, pos))
            ...
            ControlFlow::Continue(c) => {
                ...
                Err(c
                    ...
                    .map(|(pid, pattern, pos, offset)| {
                        ...
                        let split = pather.single_offset_split::<S>(sub, offset);

                        // index inner context
                        let IndexSplitResult { inner: context, .. } = pather
                            .index_secondary_path::<S, _>(
                                split
                                    .path
                                    .clone()
                                    .into_iter()
                                    .chain(std::iter::once(split.location))
                                    .collect_vec(),
                            )
                            .unwrap();
                        (
                            parent.to_child_location(SubLocation::new(pid, pos)),
                            pattern,
                            split,
                            context,
                        )
    ...
    //#[async_recursion]
    pub fn single_offset_split<S: RelativeSide<Side>>(
        &mut self,
        parent: Child,
        offset: NonZeroUsize,
    ) -> IndexSplitResult {
        ...
        match self.child_pattern_offset_splits::<S>(parent, child_patterns, offset) {
            Ok(split) => split,
            Err(splits) => self.unperfect_splits(parent, splits),
        }
    }
    fn entry_unperfect_split(
        ...
        if let Some(parent_inner) = graph
            .insert_range_in(location, Side::inner_context_range(pos))
            .ok()
        {
            ...
            if let Ok(wrapper) = graph.insert_range_in(location, Side::inner_range(pos)) {
                ...
                graph.replace_in_pattern(split_location, Side::inner_range(pos), wrapper);
                IndexSplitResult {
                    location,
                    path: vec![ChildLocation::new(inner, wrapper_pid, 1)],
                    inner: full_inner,
                }
            } else {
    ...
    fn unperfect_splits(
        ...
        if splits.len() == 1 {
            ...
            self.entry_unperfect_split(location, split, context)
        } else {
            // add contexts
            ...
            // index half patterns
            let mut graph = self.graph_mut();
            let (back, front) = (graph.insert_patterns(backs), graph.insert_patterns(fronts));
