...
use hypercontext_api::{graph::{kind::{BaseGraphKind, GraphKind}, vertex::{child::Child, location::child::ChildLocation}}, traversal::traversable::TraversableMut};

use super::side::{relative::ContextSide, IndexSide};
...
IntoIterator<Item=<Self as ContextPath>::Item, IntoIter=<Self as ContextPath>::IntoIter>
+ Debug
+ Send
+ Sync
+ Unpin
...
#[derive(Debug, Clone)]
pub struct Contexter<Side: IndexSide> {
    pub(crate) indexer: Indexer,
    _ty: std::marker::PhantomData<Side>,
}

impl<Side: IndexSide> Contexter<Side> {
...
impl<Side: IndexSide> Contexter<Side> {
    ...
    pub fn try_context_path(
        ...
        self.pather()
            .index_primary_path::<ContextSide, _>(path)
            .map(|split| (split.inner, split.location))
    }
}
