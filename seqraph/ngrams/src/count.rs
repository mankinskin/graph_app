...
pub fn test_ngrams() {
    ...
    for s in corpus {
        for (n, counts) in ngram_set(s.to_string()).into_iter().enumerate() {
            for (gr, c) in counts {
                total_counts.entry(gr).and_modify(|t|
                    *t += c
                )
                .or_insert(c);
            }
    ...
    let C_MAX: usize = 20;
    let hist = total_counts.into_iter().fold(HashMap::<usize, usize>::default(), |mut hist, (gr, c)| {
        hist.entry(c).and_modify(|t| *t += 1)
            .or_insert(1);
        hist
    });
